...
export default function SearchInput(){
    ...
    const results = useQuery(api.posts.searchPosts, term.length >= 2 ? {term, limit: 5}: "skip")
            ...
            {open && term.length >= 2 && (
                <div className="absolute top-full mt-2 rounded-md border bg-popover text-popover-foreground shadow-md outline-none animate-in fade-in-0 zoom-in-95">
                    {results === undefined ? (
                        ...
                        <div className="py-1">
                            {results.map((post) => (
                                <Link href={`/blogs/${post._id}`} key={post._id} className="flex flex-col px-4 py-2 text-sm hover:bg-accent hover:text-accent-foreground cursor-pointer" onClick={() => {setTerm(""); setOpen(false)}}>
                                    <p className="font-medium truncate">{post.title}</p>
                                    <p className="text-xs text-muted-foreground pt-1">{post.content.substring(0,60)}</p>
                                </Link>
                            ))}
                        </div>
                    )}
